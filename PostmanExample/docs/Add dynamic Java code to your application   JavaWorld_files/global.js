...
$(document).ready(function(){
	...
	//set the height of the hamburger flyout nav
	//removed 5/15/14 - we're doing flyout submenus so we don't need to do this anymore
	/* var navHeight = $('#banner nav.main').height();
	var navSections = $('#banner nav.main ul.primary li ul');
	
	for (var x=0; x < navSections.length; x++) {
		if ($(navSections[x]).height() > navHeight) {
			navHeight = $(navSections[x]).height();
		}
	}
	
	$('#banner nav.main').height(navHeight);
	*/
	...
	this.showLogin = function(showSocial) {
		...
		if (!this.Logon.isValid) {
			...
			d.load('/user/login', {'showSocial':showSocial},function() {
		        d.fadeIn(1000).css(modalCss).center();
